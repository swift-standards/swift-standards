...
extension Affine.Point where N == 2, Scalar: Real & BinaryFloatingPoint {
    ...
    public static func polar(radius: Affine.Distance, angle: Radian<Scalar>) -> Self {
        Self(
            x: Affine.X(radius._rawValue * angle.cos.value),
            y: Affine.Y(radius._rawValue * angle.sin.value)
        )
    }
...
extension Affine.Point where N == 2, Scalar: Real & BinaryFloatingPoint {
    ...
    public static func rotated(_ point: Self, by angle: Radian<Scalar>, around center: Self) -> Self {
        let translated = Self(
            x: Affine.X(point.x._rawValue - center.x._rawValue),
            y: Affine.Y(point.y._rawValue - center.y._rawValue)
        )
        let rotated = Self.rotated(translated, by: angle)
        return Self(
            x: Affine.X(rotated.x._rawValue + center.x._rawValue),
            y: Affine.Y(rotated.y._rawValue + center.y._rawValue)
        )
    }
