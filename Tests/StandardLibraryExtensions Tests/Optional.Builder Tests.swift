...
struct OptionalBuilderTests {
    ...
    struct StaticMethodTests {
        ...
        func buildPartialBlockFirst() {
            let result = Optional<Int>.Builder.buildPartialBlock(first: 42)
            #expect(result == 42)
        }

        @Test("buildPartialBlock first void")
        func buildPartialBlockFirstVoid() {
            let result = Optional<Int>.Builder.buildPartialBlock(first: ())
            #expect(result == nil)
        }

        @Test("buildPartialBlock accumulated with first non-nil")
        func buildPartialBlockAccumulatedWithFirstNonNil() {
            let result = Optional<Int>.Builder.buildPartialBlock(accumulated: 42, next: 100)
            #expect(result == 42)
        }

        @Test("buildPartialBlock accumulated with first nil")
        func buildPartialBlockAccumulatedWithFirstNil() {
            let result = Optional<Int>.Builder.buildPartialBlock(accumulated: nil, next: 100)
            #expect(result == 100)
        }
        ...
        func buildOptionalSome() {
            let inner: Int? = 42
            let result = Optional<Int>.Builder.buildOptional(inner)
            #expect(result == 42)
        }

        @Test("buildOptional none")
        func buildOptionalNone() {
            let result = Optional<Int>.Builder.buildOptional(nil)
            #expect(result == nil)
        }
        ...
        func buildArrayFirstNonNil() {
            ...
            let result = Optional<Int>.Builder.buildArray(components)
            #expect(result == 42)
        }
