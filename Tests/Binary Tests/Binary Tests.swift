...
struct EndiannessTests {
    ...
    @Test
    func `Endianness opposite`() {
        #expect(Endianness.little.opposite == .big)
        #expect(Endianness.big.opposite == .little)
    }

    @Test
    func `Endianness negation operator`() {
        #expect(!Endianness.little == .big)
        #expect(!Endianness.big == .little)
        #expect(!(!Endianness.little) == .little)
    }

    @Test
    func `Endianness CaseIterable`() {
        #expect(Endianness.allCases.count == 2)
        #expect(Endianness.allCases.contains(.little))
        #expect(Endianness.allCases.contains(.big))
    }

    @Test
    func `Endianness network is big`() {
        #expect(Endianness.network == .big)
    }

    @Test
    func `Endianness native detection`() {
