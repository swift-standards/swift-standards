...
struct `Strideable Conformance Tests` {
    ...
    func `X is Strideable`() {
        let x: Geometry<Double, Void>.X = 10
    ...
    func `Y is Strideable`() {
        let y: Geometry<Double, Void>.Y = 20
    ...
    func `Width is Strideable`() {
        let w: Geometry<Double, Void>.Width = 100
    ...
    func `Height is Strideable`() {
        let h: Geometry<Double, Void>.Height = 200
    ...
    func `Length is Strideable`() {
        let len: Geometry<Double, Void>.Length = 50
    ...
    func `Depth is Strideable`() {
        let d: Geometry<Double, Void>.Depth = 30
    ...
    func `X stride sequence`() {
        var values: [Geometry<Double, Void>.X] = []
        for x in stride(from: Geometry<Double, Void>.X(0), to: Geometry<Double, Void>.X(5), by: 1) {
            values.append(x)
        }
...
struct `Size AdditiveArithmetic Tests` {
    @Test
    func `Size addition`() {
        let a: Geometry<Double, Void>.Size<2> = .init(width: 100, height: 200)
        let b: Geometry<Double, Void>.Size<2> = .init(width: 50, height: 100)
        let sum = a + b
    ...
    func `Size subtraction`() {
        let a: Geometry<Double, Void>.Size<2> = .init(width: 100, height: 200)
        let b: Geometry<Double, Void>.Size<2> = .init(width: 30, height: 50)
        let diff = a - b
    ...
    @Test
    func `Size zero`() {
        let zero: Geometry<Double, Void>.Size<2> = .zero
        #expect(zero.width == 0)
    ...
    @Test
    func `Size negation`() {
        let size: Geometry<Double, Void>.Size<2> = .init(width: 100, height: 200)
        let neg = -size
    ...
    func `Size 3D addition`() {
        let a: Geometry<Double, Void>.Size<3> = .init(width: 10, height: 20, depth: 30)
        let b: Geometry<Double, Void>.Size<3> = .init(width: 5, height: 10, depth: 15)
        let sum = a + b
        #expect(sum.width == 15)
...
struct `EdgeInsets AdditiveArithmetic Tests` {
    ...
    func `EdgeInsets zero`() {
        let zero: Geometry<Double, Void>.EdgeInsets = .zero
        #expect(zero.top == 0)
...
struct `Scalar Wrapper Operators` {
    ...
    func `X multiplication and division`() {
        let x: Geometry<Double, Void>.X = 10
        let xTimes2: Geometry<Double, Void>.X = x * 2.0
        let twoTimesX: Geometry<Double, Void>.X = 2.0 * x
        #expect(xTimes2 == 20)
        #expect(twoTimesX == 20)
        #expect((x / 2.0) == 5)
        #expect((-x) == -10)
    ...
    func `Y multiplication and division`() {
        let y: Geometry<Double, Void>.Y = 10
        let yTimes2: Geometry<Double, Void>.Y = y * 2.0
        let twoTimesY: Geometry<Double, Void>.Y = 2.0 * y
        #expect(yTimes2 == 20)
        #expect(twoTimesY == 20)
        #expect((y / 2.0) == 5)
        #expect((-y) == -10)
    ...
    func `Width multiplication and division`() {
        let w: Geometry<Double, Void>.Width = 10
        #expect(w * 2 == 20)
        #expect(2 * w == 20)
        #expect(w / 2 == 5)
        #expect(-w == -10)
    ...
    func `Height multiplication and division`() {
        let h: Geometry<Double, Void>.Height = 10
        #expect(h * 2 == 20)
        #expect(2 * h == 20)
        #expect(h / 2 == 5)
        #expect(-h == -10)
    ...
    func `Length multiplication and division`() {
        let len: Geometry<Double, Void>.Length = 10
        #expect(len * 2 == 20)
        #expect(2 * len == 20)
        #expect(len / 2 == 5)
        #expect(-len == -10)
    ...
    func `Depth multiplication and division`() {
        let d: Geometry<Double, Void>.Depth = 10
        #expect((d * 2) == 20)
        #expect((2 * d) == 20)
        #expect((d / 2) == 5)
        #expect((-d) == -10)
